...
async function getAllFiles() {
  ...
  const researchFiles = await response.json();

  console.log("Fetched research files:", researchFiles);

  const researchFilesDiv = document.getElementById('researchFiles');
  const searchInput = document.querySelector('.search-bar input').value.toLowerCase(); // Get the lowercase value of the search input

  // Clear existing content
  researchFilesDiv.innerHTML = '';
  ...
  researchFiles.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  ...
  researchFiles.forEach(function (file) {
      ...
      if (
        ...
        fileDiv.style = "border: 2px solid #01858D; width: 700px; margin-top: 50px; border-radius: 5px 5px 5px 5px; background-color: white";
        fileDiv.innerHTML = `
        ...
        researchFilesDiv.appendChild(fileDiv);
      }
    });
  }
